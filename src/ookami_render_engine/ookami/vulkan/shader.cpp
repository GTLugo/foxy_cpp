...
namespace fx {
  ...
  public:
    ...
    : name_{ shader_create_info.shader_directory.stem().string() }
    {
      if (load_shader_code(shader_create_info)) {
        fx::Log::trace("Compiled or read in shader: {}", name_);
        create_shader_modules(device);
        fx::Log::trace("Shader \"{}\" ready.", name_);
      } else {
        fx::Log::error("Shader \"{}\" failed creation.", name_);
      }
    }
    ...
    [[nodiscard]] auto module(const Kind stage) const -> const vk::raii::ShaderModule& 
    {
      if (!shader_modules_.contains(stage)) {
        fx::Log::fatal("Shader module for {} stage not found.", *stage.to_string());
      }
    ...
    [[nodiscard]] static constexpr auto kind_to_vk_flag(const Kind kind) -> std::optional<fx::i32>
    {
      if (kind.underlying_value() == Kind::Vertex)   return static_cast<fx::i32>(vk::ShaderStageFlagBits::eVertex);
      if (kind.underlying_value() == Kind::Fragment) return static_cast<fx::i32>(vk::ShaderStageFlagBits::eFragment);
      if (kind.underlying_value() == Kind::Compute)  return static_cast<fx::i32>(vk::ShaderStageFlagBits::eCompute);
      if (kind.underlying_value() == Kind::Geometry) return static_cast<fx::i32>(vk::ShaderStageFlagBits::eGeometry);
      return std::nullopt;
    }
  private:
    static constexpr inline fx::word32 spirv_magic_number_{ 0x07230203 };

    std::string name_;
    std::unordered_map<Kind, std::vector<fx::u32>> bytecode_;
    ...
    [[nodiscard]] auto load_shader_code(const CreateInfo& create_info) -> bool
    {
      ...
      if (!exists(create_info.shader_directory)) {
        fx::Log::error("Directory {} does not exist", create_info.shader_directory.string());
      } else {
        if (is_directory(create_info.shader_directory)) {
          fx::Log::trace("Loading shader from dir: {}", name_);
          found_shader = load_from_dir(create_info);
        } else {
          fx::Log::error("Failed to load shader from dir: {}", name_);
        }
      }
    ...
    [[nodiscard]] auto load_from_dir(const CreateInfo& create_info) -> bool
    {
      ...
      for (auto [i, stage] = std::tuple<fx::u32, Kind>{ 0, static_cast<Kind::Value>(0) }; 
           i <= Kind::Max; 
    ...
    ) -> bool
    {
      ...
      if (create_info.has_kind(kind)) {
        fx::Log::trace("Looking for {}: {}", *kind.to_string(), name_);

        if (const fs::path out_shader_path{ shader_cache_dir / fs::path{ *kind.to_string() + ".spv" } }; exists(out_shader_path)) {
          fx::Log::trace("Found cached {} at location {}", *kind.to_string(), out_shader_path.string());
          ...
          fx::Log::trace("Couldn't find cached {} at location {}", *kind.to_string(), out_shader_path.string());

          if (const auto str{ fx::read_file(in_shader_path) }; str.has_value()) {
        ...
        fx::Log::trace("Skipping {}", *kind.to_string());
      }
    ...
    [[nodiscard]] auto compile_shader_type(
      ...
      const Kind stage, 
      const bool optimize
    ) -> std::optional<std::vector<fx::u32>>
    {
      fx::Log::trace("Compiling {}: {}...", *stage.to_string(), name_);
      ...
      if (result.GetCompilationStatus() != shaderc_compilation_status_success) {
        fx::Log::error("Shaderc Compile: {}", result.GetErrorMessage());
        return std::nullopt;
      }
      const std::vector<fx::u32> spv{ result.begin(), result.end() };

      if (std::ofstream spv_file{ out_file, std::ios::out | std::ios::binary }; spv_file.is_open()) {
        const std::streamsize spv_byte_count{ static_cast<fx::u32>(spv.size() * 4) };
        spv_file.write(reinterpret_cast<const char*>(spv.data()), spv_byte_count);
      } else {
        fx::Log::error("Could not write to output shader file.");
      }
    ...
    [[nodiscard]] auto preprocess(const std::string& code_str, const Kind stage) const -> std::string
    {
      ...
      if (const auto result{ compiler.PreprocessGlsl(code_str, static_cast<shaderc_shader_kind>(*stage.to_shaderc()), name_.c_str(), options) }; 
        ...
        fx::Log::error("Shaderc Preprocess: {}", result.GetErrorMessage());
        return "";
      }
    }

    [[nodiscard]] static auto read_spv(const std::filesystem::path& path) -> std::optional<std::vector<fx::word32>>
    {
      if (std::ifstream file{ path, std::ios::in | std::ios::binary }; file.is_open()) {
        const std::streamsize size{ static_cast<fx::u32>(file_size(path)) };
        fx::Log::info("Shader read file size: {}", size);

        // Read file
        std::vector<fx::byte8> byte_buffer(size);
        file.read(reinterpret_cast<char*>(byte_buffer.data()), size);

        // Convert to words
        const std::streamsize code_word_count{ static_cast<fx::u32>(byte_buffer.size() / 4) };
        const auto array_start{ reinterpret_cast<fx::u32*>(byte_buffer.data()) };
        std::vector<fx::word32> word_buffer{ array_start, array_start + code_word_count };

        if (word_buffer[0] != spirv_magic_number_) {
          fx::Log::fatal("Invalid SPIR-V binary file magic number: {}", path.string());
        }

        return word_buffer;
      }

      fx::Log::error("Failed to open file: {}", path.string());
      return std::nullopt;
    }

    void create_shader_modules(const vk::raii::Device& device)
    {
      fx::Log::trace("Building shader modules: {}...", name_);

      for (fx::u32 i{ 0 }; i <= Kind::Max; ++i) {
        if (auto stage{ static_cast<Kind::Value>(i) }; bytecode_.contains(stage)) {
          vk::ShaderModuleCreateInfo create_info{
            .codeSize = bytecode_.at(stage).size() * 4,
            .pCode = reinterpret_cast<const fx::u32*>(bytecode_.at(stage).data())
          };
      ...
      fx::Log::trace("Created shader modules: {}", name_);
    }
  };
  ...
  constexpr auto Shader::Kind::to_shaderc() const -> std::optional<fx::i32>
  {
  ...
  auto Shader::Kind::to_vk_flag() const -> std::optional<fx::i32> { return Impl::kind_to_vk_flag(*this); }
  ...
  auto Shader::kind_to_vk_flag(const Kind kind) -> std::optional<fx::i32> { return Impl::kind_to_vk_flag(kind); }
}
