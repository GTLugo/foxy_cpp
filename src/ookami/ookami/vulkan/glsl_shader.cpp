...
namespace ookami {
  ...
  GLSLShader::GLSLShader(const vk::raii::Device& device, const std::filesystem::path& file_path, BitFlags shader_bits, bool optimize)
    : pImpl_{std::make_unique<Impl>(device, file_path, shader_bits, optimize)} {}

  GLSLShader::~GLSLShader() = default;

  auto GLSLShader::module(GLSLShader::Kind kind) const -> const vk::raii::ShaderModule& { return pImpl_->module(kind); }
}